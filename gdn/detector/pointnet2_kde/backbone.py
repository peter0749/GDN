...
class Pointnet2MSG(nn.Module):
    ...
    def forward(self, pointcloud):
        ...
        x = self.FC_layer(l_features[0]).transpose(1, 2).contiguous()
        x = x.view(x.size(0), x.size(1), *self.config['output_dim']) # (B, M, 9)
        h_feat = x[..., :6] # (B, M, 6)
        xyz_feat = x[..., 6:9] + xyz # (B, M, 3) -- Learn the residual ...
        x = torch.cat((h_feat, xyz_feat), 2) # (B, M, 9)
