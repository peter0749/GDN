...
class GraspDataset(Dataset):
    def __init__(self, config, verbose=True, **kwargs):
        ...
        self.oracle_scene_path = config['oracle_data']
        self.oracle_label_path = config['oracle_label']
        temp_oracle_labels = list(sorted(list(map(lambda x: '/'.join(x.split('/')[-2:]), glob.glob(self.oracle_label_path+'/*/*.npy')))))

        # filename format: scene-[1-9][0-9]*.npy
        # select first nshot scene as training batch
        self.oracle_labels = list(filter(lambda x: int(x.split('/')[-1].split('-')[1][:-4]) <= self.num_oracle, temp_oracle_labels))
        self.config['oracle_labels'] = self.oracle_labels
    ...
    def eval(self):
        ...
        self.training = False
    def __len__(self):
        return len(self.labels) + len(self.oracle_labels)
    def __getitem__(self, item_idx):
        config = self.config
        if item_idx < len(self.labels):
            id_ = self.labels[item_idx]
            pc_path = self.scene_path
            grasp_path = self.label_path
        else:
            id_ = self.oracle_labels[item_idx - len(self.labels)]
            pc_path = self.oracle_scene_path
            grasp_path = self.oracle_label_path

        pc_scene = np.load(pc_path + '/' + id_).astype(np.float32) # (#npt, 3)
        hand_poses = np.load(grasp_path + '/' + id_).astype(np.float32) # (N, 3, 4)
