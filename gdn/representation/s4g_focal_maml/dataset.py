...
class GraspDataset(Dataset):
    ...
    def __getitem__(self, item_idx):
        ...
        for scene_id in task_list:
            ...
            Xs.append(pc_scene)
            Ys.append(poses_list)
            Ms.append(enclosed_pts_list)
        return Xs, Ys, Ms
...
class collate_fn_setup(object):
    ...
    def __call__(self, batch):
        ...
        Xpre, Ypre, Mpre = zip(*batch)
        X, F = [], []
        for b_outer in range(len(Xpre)):
            pc_batch = Xpre[b_outer]
            pose_batch = Ypre[b_outer]
            enclosed_pt_batch = Mpre[b_outer]
            pc_batch = np.stack(pc_batch).astype(np.float32)
            input_points = pc_batch.shape[1]
            feature_volume_batch = np.zeros((pc_batch.shape[0], input_points, *config['output_dim']), dtype=np.float32)
            for b in range(pc_batch.shape[0]):
                ...
                if enclosed_pt_batch[b] is None or len(enclosed_pt_batch[b]) == 0:
                    continue
                # reversed: assign with higher antipodal score
