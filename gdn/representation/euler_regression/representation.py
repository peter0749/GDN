...
class EulerRegressionRepresentation(AbstractRepresentation):
    def __init__(self, config, **kwargs):
        assert isinstance(config, dict)
        super(EulerRegressionRepresentation, self).__init__(**kwargs)
        self.config = config
    ...
    def grasp_representation(self, pose, pts, pts_index):
        ...
        rpy = Rotation.from_matrix(pose[:3,:3]).as_euler('xyz', degrees=False).astype(np.float32)
    ...
    def update_feature_volume(self, feature, index, xyz, rpy):
        '''
        feature: (M, 7)
        index: (K,), K<M
        xyz: (K, 3)
...
def retrive_from_feature_volume(pts, feature, M, hand_height, gripper_width, thickness_side, rot_th, trans_th, n_output=10, threshold=0.0, nms=False):
    ...
    for pt_idx in index:
        ...
        roll  = float(feature[pt_idx,4]) # float
        pitch = float(feature[pt_idx,5]) # float
        yaw   = float(feature[pt_idx,6]) # float
        rot = rotation_euler(roll, pitch, yaw)
