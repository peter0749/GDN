...
class GraspDatasetGPDVal(object):
    def __init__(self, config, projection=False, project_chann=3, project_size=60, **kwargs):
    ...
    def __getitem__(self, item_idx):
        ...
        def gen():
            ...
            for i in range(len(res)):
                ...
                yield grasp_pc_i # partial pc, score
            pool.close()
            pool.join()
            del pool
...
if __name__ == '__main__':
    ...
    if method == 'GPD':
        ...
        model = GPDClassifier(3, return_features=True)
    elif method == 'GPD12':
        dataset = GraspDatasetGPDVal(config, projection=True, project_chann=12, project_size=60)
        model = GPDClassifier(12, return_features=True)
    elif method == 'PointnetGPD':
        dataset = GraspDatasetGPDVal(config, projection=False, project_chann=3, project_size=60)
        model = PointNetCls(num_points=config['input_points'], input_chann=3, k=2, return_features=True)
    elif method == 'PointnetGPDMC':
        dataset = GraspDatasetGPDVal(config, projection=False, project_chann=3, project_size=60)
        model = PointNetCls(num_points=config['input_points'], input_chann=3, k=3, return_features=True)
    else:
        raise NotImplementedError
    ...
    model.eval()
    with torch.no_grad():
        for cloud_id, candidate, generator in dataset:
            logits = np.full(len(candidate), -np.inf)
            features = np.zeros((len(candidate), feature_dim), dtype=np.float32)
            pbar = tqdm(total=len(candidate))
            pbar.set_description('%s_%s'%(cloud_id[0], cloud_id[1]))
            bad_input_frac = 0
            gen = generator()
            batch = []
            batch_idx = []
            for i, pc in enumerate(gen):
                ...
                if len(batch)>=config['batch_size']:
                    ...
                    feat = out[-1].cpu().numpy() # feature
                    out = out[0][:,-1].cpu().numpy() # (#batch,)
                    logits[batch_idx] = out
                    features[batch_idx] = feat
                    batch = []
                    batch_idx = []
            if len(batch)>0:
                out = model(torch.from_numpy(np.concatenate(batch, axis=0)).float().cuda())
            ...
            bad_input_frac /= float(len(candidate))
            pbar.close()
            prefix = output_dir + '/' + cloud_id[0]
            if not os.path.exists(prefix):
                os.makedirs(prefix)
            np.save(prefix+'/'+cloud_id[1]+'_features.npy', features)
            print('Processed: {}_{} {:s}, bad_r: {:.2f}'.format(cloud_id[0], cloud_id[1], str(candidate.shape), bad_input_frac))
