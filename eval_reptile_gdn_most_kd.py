...
def parse_args():
    ...
    parser.add_argument("--kd_alpha", type=float, default=0.9, help="Alpha of distillation loss (alpha * CE + (1-alpha) * KD)")
...
if __name__ == '__main__':
    ...
    if args.ntrain > 0:
        ...
        Ys_teacher = torch.zeros_like(Ys)
        ...
        with torch.no_grad():
            for start in range(0, len(Xs), args.batch_size):
                ...
                n_fold = 10
                y_teacher = model(X).cpu()
                for _ in range(n_fold-1):
                    y_teacher += model(X).cpu()
                y_teacher = y_teacher / float(n_fold)
                Ys_teacher[start:start+args.batch_size] = y_teacher
        ...
        for _ in tqdm(range(args.nepoch)):
            ...
            for start in range(0, len(Xs), args.batch_size):
                ...
                X = Xs[mbinds].cuda()
                Y = Ys[mbinds].cuda()
                Y_teacher = Ys_teacher[mbinds].cuda()
                optimizer.zero_grad()
                pred = model(X)
                loss, ce_loss, kd_loss, consist_loss = loss_function(pred, Y, Y_teacher)[:4]
                print("LOSS: %.2f CE: %.4f, KD: %.4f, REG: %.4f"%(loss.item(), ce_loss, kd_loss, consist_loss), flush=True)
                loss.backward()
                optimizer.step()
