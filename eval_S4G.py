    ...
    with torch.no_grad():
        while True:
            try:
                ...
                break
            pc, cloud_id_batch = batch
            pred = model(pc.cuda()).cpu().numpy()
            pc_npy = pc.cpu().numpy()
            pred_poses = representation.retrive_from_feature_volume_batch(pc_npy, pred, n_output=300, threshold=-np.inf, nms=True)
