        ...
        for b in range(len(pose_idx_pairs_batch)):
            ...
            for n, (pose, ind) in enumerate(pose_idx_pairs_batch[b]):
                ...
                (xyz,
                 ...
                 new_ind)
                unique_grasp_key = (tuple(new_ind), pitch_index, yaw_index)
            ...
            feature_volume = np.zeros((config['subsample_levels'][config['output_layer']],
                                       ...
                                       config['n_yaw'],
                                       8), dtype=np.float32)
            for n, ((new_ind, pitch_index, yaw_index), (xyz, roll, pitch_residual, yaw_residual)) in enumerate(sample_dict.items()):
                representation.update_feature_volume(feature_volume, new_ind, xyz, roll, pitch_index, pitch_residual, yaw_index, yaw_residual)
