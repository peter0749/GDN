...
def parse_args():
    ...
    parser.add_argument("--confidence_threshold", type=float, default=0.3, help="Threshold for postive pseudo-label")
    parser.add_argument("--T", type=int, default=800, help="Number of bootstrap sampling.")
...
if __name__ == '__main__':
    ...
    if args.ntrain > 0:
        ...
        # f(x) <- 0
        # n(x) <- 0
        Hs_r = torch.zeros_like(Ys) # store numerator (bagging features)
        ...
        for t in tqdm(range(args.T)):
            ...
            with torch.no_grad():
                for start in range(0, len(Xs), args.batch_size):
                    ...
                    # For any x in M = U \ u update
                    M = ~bootstrap_mask[start:start+args.batch_size].cuda()
                    h = model(X)[1]
                    # f(x) <- f(x) + f_t(x)
                    # n(x) <- n(x) + 1
                    Hs_r[start:start+args.batch_size][M] += h[M].cpu()
                    Ps_r[start:start+args.batch_size][M] += 1.
        with torch.no_grad():
            # s(x) = f(x) / n(x) for x in U
            Hs_r = Hs_r / Ps_r.clamp(min=1.0) # average on OOB features and prevent divide by 0
            Ys_r = base_model.activation_layer(Hs_r)[0] # Apply activation
            Ys_r[...,0] = torch.sigmoid(Ys_r[...,0])
            logit = Ys_r[...,0].numpy().reshape(-1)
            logit_dist = np.histogram(logit, bins=np.linspace(0,1,11)[:10])
            Ys_r[...,0] = (Ys_r[...,0]>args.confidence_threshold).float()
            # combine true label and pseudo-label
            Ys_pseudo = (Ys[...,0]<=0) & (Ps_r[...,0]>0) & (Ys_r[...,0]>0) # (not positive sample) & (has pseudo-labels)
            Ys[Ys_pseudo,:] = Ys_r[Ys_pseudo,:]
            print("Generated %.1f pseudo-labels per batch."%(torch.sum(Ys_pseudo) / Ys_pseudo.size(0)), flush=True)
            print("Logit dist:", flush=True)
            print(logit_dist, flush=True)
